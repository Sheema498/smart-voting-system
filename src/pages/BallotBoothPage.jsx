...
export const BallotBoothPage = () => {
  ...
  const getInitialStep = () => {
    if (location.pathname.endsWith('/instructions')) return 0;
    if (location.pathname.endsWith('/candidates')) return 1;
    if (location.pathname.endsWith('/review')) return 2;
    if (location.pathname.endsWith('/confirmation')) return 3;
    return 1;
  };

  // Wizard steps: 0: Instructions, 1: Select, 2: Review, 3: Sign & Submit
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs sm:text-sm text-slate-600">
          Voter ID: <span className="font-mono font-semibold text-slate-800">{user?.voterId || 'VS-984210-2026'}</span> • Registered in {user?.district || 'District 4 - Central'}
        </p>
      </div>

      {/* STEP 0: OFFICIAL BALLOTING INSTRUCTIONS */}
      {step === 0 && (
        <div className="space-y-6 bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center space-x-3 text-primary-700">
            ...
            <h3 className="text-lg font-bold text-slate-900">Official Democratic Guidelines</h3>
          </div>
          <p className="text-sm text-slate-600 leading-relaxed">
            Welcome to the certified digital balloting booth for <strong>{election.title}</strong>. Please review the following democratic rules before marking your candidate:
          </p>
          <div className="grid sm:grid-cols-2 gap-4 text-xs">
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-1">
              <span className="font-bold text-slate-900 block">Confidential & Secret Ballot</span>
              <p className="text-slate-600">Your choice is permanently decoupled from your identity. Nobody can see who you voted for.</p>
            </div>
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-1">
              <span className="font-bold text-slate-900 block">Single Non-Transferable Vote</span>
              <p className="text-slate-600">You are entitled to select exactly one candidate or proposition. Ballots cannot be retracted once signed.</p>
            </div>
          </div>
          <div className="pt-4 flex justify-end">
            <Button variant="primary" onClick={() => setStep(1)}>
              Proceed to Candidate Selection <ArrowRight className="w-4 h-4 ml-1.5" />
            </Button>
          </div>
        </div>
      )}
